...
export const ChecklistNavigation = Extension.create({
    ...
    addKeyboardShortcuts() {
        return {
            ArrowUp: ({ editor }) => {
                ...
                if ($from.parentOffset === 0) return true; // блокируем
                return false;
            },
    ...
    addProseMirrorPlugins() {
        ...
        // Находит все диапазоны блоков checklist в документе
    ...
    addNodeView() {
        return ({ node, editor, getPos }) => {
            ...
            return {
                ...
                destroy: cleanup,
                // Update метод вызывается при обновлении узла - восстанавливаем стили кнопки
                update: () => {
                    const hasFocus = isFocusInThisNode();
                    setDeleteButtonVisible(hasFocus);
                    return true;
