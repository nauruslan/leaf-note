...
@php
    ...
    $typeLabel = $isFolder ? 'Папка' : ($isChecklist ? 'Список' : 'Заметка');
    $deletedAt = $isFolder ? $item->moved_to_trash_at : $item->moved_to_trash_at;
@endphp
...
<div
    ...
    <div class="flex items-start justify-between mb-4">
        <div class="flex flex-1 flex-col">
            <h3 class="font-bold text-lg text-gray-900">{{ $typeLabel }}: {{ $item->title ?: 'Без названия' }}</h3>
        ...
        @if ($isFolder)
            <div class="flex flex-col items-center justify-center w-full">
                <p class="text-sm text-gray-500">
                    {{ $item->trashedNotes->count() }}
                    {{ trans_choice('заметка|заметки|заметок', $item->trashedNotes->count()) }} в папке
                </p>
            </div>
        @elseif ($isChecklist)
